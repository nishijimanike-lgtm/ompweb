...
test("validates provider discovery blocks", () => {
  const base = {
    providers: {
      p: {
        baseUrl: "https://api.example.com/v1",
  ...
  // A valid openai-models-list discovery passes.
  validateModelsConfig({
    providers: { p: { ...base.providers.p, discovery: { type: "openai-models-list" } } },
  });
  validateModelsConfig({
    providers: {
      p: {
        ...base.providers.p,
        discovery: { type: "openai-models-list", injectV1: true, timeoutMs: 10_000 },
      },
    },
  });

  // proxy discovery is valid without a provider-level api.
  validateModelsConfig({
    providers: { p: { baseUrl: base.providers.p.baseUrl, auth: "none", discovery: { type: "proxy" } } },
  });

  // Non-proxy discovery without a provider-level api is rejected.
  assert.throws(
    () => validateModelsConfig({ providers: { p: { baseUrl: base.providers.p.baseUrl, auth: "none", discovery: { type: "ollama" } } } }),
    /discovery\.type \"ollama\" requires the provider-level \"api\"/,
  );

  // injectV1 only makes sense on openai-models-list.
  assert.throws(
    () => validateModelsConfig({ providers: { p: { ...base.providers.p, discovery: { type: "litellm", injectV1: true } } } }),
    /injectV1 is only supported for discovery\.type \"openai-models-list\"/,
  );

  // Unknown discovery types are rejected.
  assert.throws(
    () => validateModelsConfig({ providers: { p: { ...base.providers.p, discovery: { type: "wat" } } } }),
    /discovery\.type \"wat\" is not valid/,
  );

  // timeoutMs must be a positive finite number.
  assert.throws(
    () => validateModelsConfig({ providers: { p: { ...base.providers.p, discovery: { type: "proxy", timeoutMs: -5 } } } }),
    /discovery\.timeoutMs must be a positive finite number/,
  );
  assert.throws(
    () => validateModelsConfig({ providers: { p: { ...base.providers.p, discovery: { type: "proxy", timeoutMs: Number.NaN } } } }),
    /discovery\.timeoutMs must be a positive finite number/,
  );
});

test("serializes discovery blocks through the editor round-trip", () => {
  const config = {
    providers: {
      p: {
  ...
  const out = serializeModelsConfig(config);
  assert.match(out, /discovery:/);
  assert.match(out, /type: openai-models-list/);
  assert.match(out, /injectV1: false/);
});
